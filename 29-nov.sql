...
UPDATE soap SET soapid=500 where soapname='santhoor';

SELECT * FROM soap where soapname='santhoor' AND quality=1; /* AND command  */

SELECT * FROM soap where color='white' OR soapstatus='good';  /* OR command */

SELECT * FROM soap where soapid IN (500,506,508);     /* IN command */

SELECT *FROM SOAP where soapid NOT IN (502,505,509);  /* NOTIN command */

UPDATE soap SET quality=0  where soapname='baby';   /* chainging value of particular coloume */

DELETE  FROM soap where soapid=500  or soapstatus='nice';

SELECT * FROM soap where soapid=502 AND quality=1 AND price=70;
SELECT * FROM soap where manufacture_date=2006 OR weight=70 OR soapstatus='dry';
SELECT *FROM soap where weight IN (50,70); 
SELECT *FROM soap where color NOT IN ('YELLOW');
